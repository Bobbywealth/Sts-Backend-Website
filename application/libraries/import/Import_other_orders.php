...
class Import_other_orders extends App_import
{
        ...
        foreach ($this->getRows() as $rowNumber => $row) {
            ...
            for ($i = 0; $i < $totalDatabaseFields; $i++) {
                ...
                if ($databaseFields[$i] == 'name' && empty($row[$i])) {
                    $row[$i] = '/';
                } elseif ($databaseFields[$i] == 'P_country') {
            ...
            if (count($insert) > 0) {
                ...
                if (!$this->isSimulation()) {
                    ...
                    $this->ci->db->insert(db_prefix() . 'other_services', $insert);
                ...
                $this->handleCustomFieldsInsert($id, $row, $i, $rowNumber, 'other_services');
            }
    ...
    {
        return site_url('clients/other_import');
    ...
    private function isDuplicateLead($data)
    {
        foreach ($this->uniqueValidationFields as $field) {
            if ((isset($data[$field]) && $data[$field] != '')
                && total_rows(db_prefix() . 'other_services', [$field => $data[$field]]) > 0) {
                return true;
            }
        }
    ...
    private function formatValuesForSimulation($values)
    {
        foreach ($values as $column => $val) {
            if ($column == 'P_country' && !empty($val) && is_numeric($val)) {
