...
class Import_physical_orders extends App_import
{
        ...
        foreach ($this->getRows() as $rowNumber => $row) {
            ...
            for ($i = 0; $i < $totalDatabaseFields; $i++) {
                ...
                if ($databaseFields[$i] == 'name' && empty($row[$i])) {
                    $row[$i] = '/';
                } elseif ($databaseFields[$i] == 'e_country') {
            ...
            if (count($insert) > 0) {
                ...
                if (!$this->isSimulation()) {
                    ...
                    $this->ci->db->insert(db_prefix() . 'physical_eviction', $insert);
                ...
                $this->handleCustomFieldsInsert($id, $row, $i, $rowNumber, 'physical_eviction');
            }
    ...
    {
        return site_url('clients/physical_import');
    ...
    private function isDuplicateLead($data)
    {
        foreach ($this->uniqueValidationFields as $field) {
            if ((isset($data[$field]) && $data[$field] != '')
                && total_rows(db_prefix() . 'physical_eviction', [$field => $data[$field]]) > 0) {
                return true;
            }
        }
    ...
    private function formatValuesForSimulation($values)
    {
        foreach ($values as $column => $val) {
            if ($column == 'e_country' && !empty($val) && is_numeric($val)) {
