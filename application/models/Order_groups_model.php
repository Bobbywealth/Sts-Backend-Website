...
class Order_groups_model extends App_Model
{
    ...
    {
        $this->db->insert(db_prefix().'orders_group', $data);
    ...
    */
    ...
    public function get_groups($id = '')
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix().'orders_group')->row();
        }
    ...
    public function edit($data)
    {
        ...
        $this->db->update(db_prefix().'orders_group', [
            'name' => $data['name'],
        ]);
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix().'orders_group');
        if ($this->db->affected_rows() > 0) {
            $this->db->where('id', $id);
            $this->db->delete(db_prefix().'orders_group');
        ...
        if (sizeof($customer_groups) > 0) {
            ...
            if (isset($groups_in)) {
                foreach ($groups_in as $group) {
                    ...
                    $_exists = $this->db->get(db_prefix().'orders_group')->row();
                    if (!$_exists) {
                        if (empty($group)) {
                            continue;
                        }
                        $this->db->insert(db_prefix().'orders_group', [
                            'customer_id' => $id,
                            'groupid'     => $group,
                        ]);
                        if ($this->db->affected_rows() > 0) {
