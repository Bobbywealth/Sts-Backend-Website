...
class Clients extends ClientsController
{
    ...
    public function eviction_filling()
    {
       $data['client_id']= get_client_user_id();
        ...
        if ($this->input->post()) {
            ...
            $e_id= $this->clients_model->eviction_filling($postdata);   
            if ($e_id) {
                if (count($_FILES)>0)
                 {
            ...
            redirect(site_url('clients/eviction_filling/'));          
         }
    }
    public function physical_eviction(){
        $data['client_id']= get_client_user_id();
        $data['group'] = $this->order_model->get_order();
        $this->data($data);
        $this->view('eviction/physical_eviction');
        $this->layout(); 
        if($this->input->post()){
            $postdata=$this->input->post();
        ...
        redirect(site_url('clients/physical_eviction'));    
        }
    }
    public function other_services (){
        ...
        $data['group'] = $this->order_model->get_order();
        $this->data($data);
        $this->view('eviction/other_services');
        $this->layout(); 
        if($this->input->post()){
            $postdata=$this->input->post();
            if(isset($postdata['eviction_fees'])) 
        ...
        redirect(site_url('clients/other_services'));    
        }
    }
    public function import()
    {
        $dbFields = $this->db->list_fields(db_prefix() . 'eviction_filling');
        array_push($dbFields, 'tags');
        $this->load->library('import/import_orders', [], 'import');
        $this->import->setDatabaseFields($dbFields)
        ->setCustomFields(get_custom_fields('eviction_filling'));
        ...
        if ($this->input->post()
            ...
            if (!$this->import->isSimulation()) {
                ...
                redirect(site_url('clients/import')); 
            }
               
        }
        $data['title'] = _l('import');
        $this->data($data);
        $this->view('eviction/import');
        $this->layout(); 

    }
    public function physical_import()
    {
        $dbFields = $this->db->list_fields(db_prefix() . 'physical_eviction');
        array_push($dbFields, 'tags');
        $this->load->library('import/import_physical_orders', [], 'import');
        $this->import->setDatabaseFields($dbFields)
        ->setCustomFields(get_custom_fields('physical_eviction'));
        ...
        if ($this->input->post()
            ...
            if (!$this->import->isSimulation()) {
                set_alert('success', _l('import_total_imported', $this->import->totalImported()));
                redirect(site_url('clients/physical_import'));   
            }
        }
        $data['title'] = _l('import');
        $this->data($data);
        $this->view('eviction/physical_import');
        $this->layout();   
    }
    public function other_import()
    {
        $dbFields = $this->db->list_fields(db_prefix() . 'other_services');
        array_push($dbFields, 'tags');
        $this->load->library('import/import_other_orders', [], 'import');
        $this->import->setDatabaseFields($dbFields)
        ->setCustomFields(get_custom_fields('other_services'));
        ...
        if ($this->input->post()
            ...
            if (!$this->import->isSimulation()) {
                set_alert('success', _l('import_total_imported', $this->import->totalImported()));
                redirect(site_url('clients/other_import'));   
            }
        }
        $data['title'] = _l('import');
        $this->data($data);
        $this->view('eviction/other_import');
        $this->layout();
