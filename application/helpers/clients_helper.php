        ...
        foreach($_FILES as $key => $value){
            if (isset($_FILES[$key]['name']) && $_FILES[$key]['name'] != '') {
                ...
                if (!empty($tmpFilePath) && $tmpFilePath != '') {
                    ...
                    $filename    = unique_filename($path, $_FILES[$key]['name']);
                    $newFilePath = $path . $filename;
                    // Upload the file into the video library uploads dir
                    if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                        $CI             = &get_instance();
                        $CI->db->where('evic_id', $e_id);
                        $CI->db->update(db_prefix() . 'eviction_filling', [
                            $key => $filename,
                        ]);
...
function get_eviction($evic_id)
{
    $CI = &get_instance();
    $CI->db->where('evic_id', $evic_id);
    $admins = $CI->db->get(db_prefix().'eviction_filling')->row();
